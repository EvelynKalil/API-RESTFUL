...
class TestErrorHandlers:
    ...
    FORBIDDEN_DETAIL = "Forbidden test"
    ...
    def test_not_found_handler(self):
        ...
        response = client.get(
            f"{BASE_URL_MESSAGES}/{self.SESSION_ID_INVALID}",
    ...
    def test_invalid_sender_handler(self):
        ...
        response = client.get(
            f"{BASE_URL_MESSAGES}/{self.SESSION_ID_VALID}?sender={INVALID_SENDER}",
            headers=API_KEY_HEADER,
        )
        assert response.status_code == STATUS_BAD_REQUEST
        data = response.json()
    ...
    def test_request_validation_error(self):
        ...
        response = client.get(
            f"{BASE_URL_MESSAGES}/{self.SESSION_ID_VALID}?limit={self.INVALID_LIMIT_VALUE}",
            headers=API_KEY_HEADER,
        )
        assert response.status_code == STATUS_BAD_REQUEST
        data = response.json()
        assert data[FIELD_ERROR][FIELD_CODE] == ERROR_CODE_MISSING_FIELD
        assert self.FIELD_NAME_MISSING not in data[FIELD_ERROR][FIELD_DETAILS]
    ...
    def test_duplicate_message_id_handler(self):
        ...
        r1 = client.post(BASE_URL_MESSAGES, json=payload, headers=API_KEY_HEADER)
        assert r1.status_code == STATUS_CREATED

        r2 = client.post(BASE_URL_MESSAGES, json=payload, headers=API_KEY_HEADER)
    ...
    def test_http_exception_non_401(self):
        """Should raise non-401 HTTPException (to cover raise exc)."""
        @app.get(self.TEST_FORCE_HTTP_EXCEPTION_ENDPOINT)
        def force_http_exception():
            raise HTTPException(
